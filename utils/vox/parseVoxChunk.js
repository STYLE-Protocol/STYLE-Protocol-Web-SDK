...
export const readDict = (contentData) => {
  const dict = {}
  let i = 0
...
export const parseVoxChunk = (id, contentData) => {
  ...
  if (id === 'RGBA')
    return {
      values: tokens.map((c) => ({ r: c[0], g: c[1], b: c[2], a: c[3] })),
    }
  ...
  if (id === 'nSHP') {
    ...
    for (let i = 0; i < obj.numModels; i++) {
      obj.models.push([
        readInt(contentData.splice(0, 4)),
        readDict(contentData),
      ])
    }
    return obj
  }
  ...
  if (id === 'NOTE') {
    ...
    for (let i = 0; i < obj.numColorNames; i++) {
      ...
      obj.colorNames.push(
        readString(flatten(contentData.splice(0, stringLength)))
      )
    }
    return obj
  }
